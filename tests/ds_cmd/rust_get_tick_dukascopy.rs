...
fn gen_src_cmd() {
    ...
    let r = do_gen_src(
        ...
        test_out_path!("rust-get-tick-dukascopy", "cmd.help.rs.txt"),
        test_out_path!("rust-get-tick-dukascopy", "cmd.match.rs.txt"),
    );
    ...
    for v in vec_optstr {
        let v_meta_type = match v.lon.as_str() {
            ...
            */
            ...
            "from" => MetaType::Other("opt_fromto_date".to_string()),
            "to" => MetaType::Other("opt_fromto_date".to_string()),
            _ => v.meta_type.clone(),
        };
        //
