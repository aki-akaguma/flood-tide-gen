...
#[test]
fn gen_src_cmd() {
    assert!(std::fs::create_dir_all(test_out_path!("rust-pdumpfs")).is_ok());
    //
    let r = do_gen_src(
        Pasc::Void,
        test_in_path!("ds-cmd/rust-pdumpfs-cmd.txt"),
        Some(test_out_path!("rust-pdumpfs", "cmd.help.rs.txt")),
        Some(test_out_path!("rust-pdumpfs", "cmd.match.rs.txt")),
        |opt_str| {
    ...
    //
    compare_out_res!("rust-pdumpfs", "ds-cmd/rust-pdumpfs", "cmd.help.rs.txt");
    compare_out_res!("rust-pdumpfs", "ds-cmd/rust-pdumpfs", "cmd.match.rs.txt");
}
